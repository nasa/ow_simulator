...
def dig_circular(move_arm, move_limbs, robot, moveit_fk, args):
  ...
  if not parallel:
    ...
    plan_c = change_joint_value(move_arm, cs, start_state, constants.J_HAND_YAW,  math.pi/2.2)
    ...
    cs, start_state, end_pose = calculate_joint_state_end_pose_from_plan_arm (robot, circ_traj, move_arm, moveit_fk)
    plan_e = change_joint_value(move_arm, cs, start_state, constants.J_HAND_YAW, -0.29*math.pi)
    ...
    cs, start_state, end_pose = calculate_joint_state_end_pose_from_plan_arm (robot, circ_traj, move_arm, moveit_fk)
    plan_d = change_joint_value(move_arm, cs, start_state, constants.J_DIST_PITCH, -19.0/54.0*math.pi)
    circ_traj = cascade_plans (circ_traj, plan_d)
    ...
    plan_f = change_joint_value(move_arm, cs, start_state, constants.J_DIST_PITCH, dist_now + 2*math.pi/3)
