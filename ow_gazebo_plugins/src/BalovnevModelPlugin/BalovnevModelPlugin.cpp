...
void BalovnevModelPlugin::Load(physics::ModelPtr model, sdf::ElementPtr sdf)
{
  ...
  m_sub_dig_phase = m_node_handle->subscribe(TOPIC_DIG_PHASE,
    1, &BalovnevModelPlugin::onDigPhaseMsg, this);
...
{
  if (!m_digging || (m_horizontal_force == 0.0 && m_vertical_force == 0.0)) {
...
void BalovnevModelPlugin::onDigPhaseMsg(
  const ow_dynamic_terrain::scoop_dig_phase::ConstPtr &msg)
{
  m_digging = msg->digging;
  if (msg->phase == ow_dynamic_terrain::scoop_dig_phase::NOT_DIGGING) {
    resetForces();
    resetDepth();
  }
}
